...
class Chatbot(Resource):
    def post(self):
        try:
            ...
            input_lang = translate_client.detect_language(prediction_input)['language']

            with open('special_words.json', 'r') as f:
                special_words = json.load(f)    

            if prediction_input == "" or prediction_input in special_words:
                input_lang='en'

            if input_lang in ['en', 'ur']:            
                if input_lang != 'en':
                    prediction_input = translate_client.translate(prediction_input, target_language='en')['translatedText']
                ...
                if input_lang != 'en':
                    ...
                    translated_response = translate_client.translate(response, target_language=input_lang)['translatedText']
                    if input_lang=='ur':
                        response = translated_response.replace('||', '\n')
                    else:
                        response = translated_response.replace(delimiter, '\n')

            else:
                response="Doesn't Support the language you are using."
                if input_lang != 'en':
                    response = translate_client.translate(response, target_language=input_lang)['translatedText']
